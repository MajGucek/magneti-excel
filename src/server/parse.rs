...
pub fn get_existing_files(folder: PathBuf) -> Vec<PathBuf> {
    FILE_NAMES.iter().map(|name| folder.join(name))
...
pub fn parse_all_files(files: Vec<PathBuf>, db_manager: &Arc<Mutex<DBManager>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = parse_sifrant_file(sifrant_file).and_then(|rows| {
        ...
        let db = db_manager.lock().map_err(|err| err.to_string())?;
        db.store_sifrant_to_db(rows)
    ...
    let _ = parse_dobavitelji_file(dobavitelji_file).and_then(|rows| {
        ...
        let db = db_manager.lock().map_err(|err| err.to_string())?;
        db.store_dobavitelji_to_db(rows)
    }).inspect_err(|e| log::error!("{}", e.to_string()));
    ...
    let _ = parse_razpolozljiva_zaloga_file(zaloga100_file).and_then(|rows| {
        ...
        let db = db_manager.lock().map_err(|err| err.to_string())?;
        db.store_razpolozljive_zaloge_to_db(rows)
    }).inspect_err(|e| log::error!("{}", e.to_string()));
    ...
    let _ = parse_poraba_file(poraba_file).and_then(|row_data| {
        ...
        let db = db_manager.lock().map_err(|err| err.to_string())?;
        db.store_poraba_to_db(row_data)
    }).inspect_err(|e| log::error!("{}", e.to_string()));
    ...
    let _ = parse_nabava_file(prevzemi_file).and_then(|row_data| {
        ...
        let db = db_manager.lock().map_err(|err| err.to_string())?;
        db.store_nabava_to_db(row_data)
    }).inspect_err(|e| log::error!("{}", e.to_string()));
...
pub fn parse_dobavitelji_file(path: PathBuf) -> Result<Vec<DobaviteljRow>, Box<dyn std::error::Error>> {
    ...
    for row in range.rows().skip(1) {
        ...
        let enota_cene = row.get(20)
            .and_then(DataType::get_float)
            .unwrap_or(0.);
        let cena = if enota_cene != 0. { neto_cena / enota_cene } else { 0. };
